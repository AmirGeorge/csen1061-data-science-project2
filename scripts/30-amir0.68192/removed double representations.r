...
trainRoamDf <-
  trainRoamDf %>% mutate(
    X206_SESSION_COUNT = X206_SESSION_COUNT - R206_SESSION_COUNT,
...
monthly_usage_columns <-
  c('X206_USAGE','X207_USAGE','X208_USAGE','X209_USAGE','X210_USAGE')
weekdays <-
...
mid <- 2200
train.df <-
  train.df %>% rowwise() %>% mutate(
    diff1 = X207_USAGE - X206_USAGE, diff2 = X208_USAGE - X207_USAGE, diff3 =
      X209_USAGE - X208_USAGE,diff4 = X210_USAGE - X209_USAGE, always_increasing = (diff1 > 0 &
                                                                                      diff2 > 0 &
                                                                                      diff3 > 0 &
                                                                                      diff4 > 0), above_mid = (
                                                                                        min(X206_USAGE,X207_USAGE,X208_USAGE,X209_USAGE,X210_USAGE) > mid
                                                                                      )
  )
test.df <-
  test.df %>% rowwise() %>% mutate(
    diff1 = X207_USAGE - X206_USAGE, diff2 = X208_USAGE - X207_USAGE, diff3 =
      X209_USAGE - X208_USAGE,diff4 = X210_USAGE - X209_USAGE, always_increasing = (diff1 > 0 &
                                                                                      diff2 > 0 &
                                                                                      diff3 > 0 &
...
for (name in weekdays) {
  ...
  col_name_sessions_count <-
    paste('local_',name,'_sessions_count',sep = '')
  filtered_data <-
    a %>% filter(ROAMING_FLAG == "LOCAL" & DAY_NAME == name)
  
  exp <- paste(
    "filtered_data %>% group_by(CONTRACT_KEY) %>%
  ...
  exp <-
    paste(
      "train.df <- train.df %>% mutate(",col_name_usage,"= ifelse(is.na(",col_name_usage,"),X210_USAGE/7,",col_name_usage,"))", sep = ""
    )
  eval(parse(text = exp))
  ...
  exp <-
    paste(
      "train.df <- train.df %>% mutate(",col_name_sessions_count,"= ifelse(is.na(",col_name_sessions_count,"),X210_SESSION_COUNT/7,",col_name_sessions_count,"))", sep = ""
    )
  eval(parse(text = exp))
  ...
  exp <-
    paste(
      "test.df <- test.df %>% mutate(",col_name_usage,"= ifelse(is.na(",col_name_usage,"),X210_USAGE/7,",col_name_usage,"))", sep = ""
    )
  eval(parse(text = exp))
  
  exp <-
    paste(
      "test.df <- test.df %>% mutate(",col_name_sessions_count,"= ifelse(is.na(",col_name_sessions_count,"),X210_SESSION_COUNT/7,",col_name_sessions_count,"))", sep = ""
    )
  eval(parse(text = exp))
  ...
  col_name_sessions_count <-
    paste('roaming_',name,'_sessions_count',sep = '')
  
  filtered_data <-
    a %>% filter(ROAMING_FLAG == "ROAMING" & DAY_NAME == name)
  exp <- paste(
    "filtered_data %>% group_by(CONTRACT_KEY) %>%
    summarise(", col_name_usage, "= sum(TOTAL_CONSUMPTION),",col_name_sessions_count,"=sum(NO_OF_SESSIONS) )", sep = ""
  )
  ...
  exp <-
    paste(
      "train.df <- train.df %>% mutate(",col_name_usage,"= ifelse(is.na(",col_name_usage,"),R210_USAGE/7,",col_name_usage,"))", sep = ""
    )
  eval(parse(text = exp))
  ...
  exp <-
    paste(
      "train.df <- train.df %>% mutate(",col_name_sessions_count,"= ifelse(is.na(",col_name_sessions_count,"),R210_SESSION_COUNT/7,",col_name_sessions_count,"))", sep = ""
    )
  eval(parse(text = exp))
  ...
  exp <-
    paste(
      "test.df <- test.df %>% mutate(",col_name_usage,"= ifelse(is.na(",col_name_usage,"),R210_USAGE/7,",col_name_usage,"))", sep = ""
    )
  eval(parse(text = exp))
  ...
  exp <-
    paste(
      "test.df <- test.df %>% mutate(",col_name_sessions_count,"= ifelse(is.na(",col_name_sessions_count,"),R210_SESSION_COUNT/7,",col_name_sessions_count,"))", sep = ""
    )
  eval(parse(text = exp))
}
...
train.df <- train.df %>% rowwise() %>% mutate(
  weekday_local_usage = sum(
    c(
      local_Sunday_usage,local_Monday_usage,local_Tuesday_usage,local_Wednesday_usage,local_Thursday_usage
    )
  )
  ,
  weekday_roaming_usage = sum(
    c(
      roaming_Sunday_usage,roaming_Monday_usage,roaming_Tuesday_usage,roaming_Wednesday_usage,roaming_Thursday_usage
    )
  )
  ,
  weekday_local_session_count = sum(
    c(
      local_Sunday_sessions_count,local_Monday_sessions_count,local_Tuesday_sessions_count,local_Wednesday_sessions_count,local_Thursday_sessions_count
    )
  )
  ,
  weekday_roaming_session_count = sum(
    c(
      roaming_Sunday_sessions_count,roaming_Monday_sessions_count,roaming_Tuesday_sessions_count,roaming_Wednesday_sessions_count,roaming_Thursday_sessions_count
    )
  )
  ,
  weekend_local_usage = sum(
    c(
      local_Friday_usage,local_Saturday_usage
    )
  )
  ,
  weekend_roaming_usage = sum(
    c(
      roaming_Friday_usage,roaming_Saturday_usage
    )
  )
  ,
  weekend_local_session_count = sum(
    c(
      local_Friday_sessions_count,local_Saturday_sessions_count
    )
  )
  ,
  weekend_roaming_session_count = sum(
    c(
      roaming_Friday_sessions_count,roaming_Saturday_sessions_count
    )
  )
)
...
train.df <- train.df %>% rowwise() %>% mutate(
  mean_usage = mean(c(
    X206_USAGE,X207_USAGE,X208_USAGE,X209_USAGE,X210_USAGE
  )),
  ...
  median_usage = median(c(
    X206_USAGE,X207_USAGE,X208_USAGE,X209_USAGE,X210_USAGE
  )),
  
  mean_sessions_count = mean(
    c(
      X206_SESSION_COUNT,X207_SESSION_COUNT,X208_SESSION_COUNT,X209_SESSION_COUNT,X210_SESSION_COUNT
    )
  ),
  
  median_sessions_count = median(
    c(
      X206_SESSION_COUNT,X207_SESSION_COUNT,
      X208_SESSION_COUNT,X209_SESSION_COUNT,X210_SESSION_COUNT
    )
  ),
  
  sd_sessions_count = sd(
    c(
      X206_SESSION_COUNT,X207_SESSION_COUNT,X208_SESSION_COUNT,X209_SESSION_COUNT,X210_SESSION_COUNT
    )
  )
...
myResult <-
  data.frame(CONTRACT_KEY = test.df$CONTRACT_KEY, PREDICTED_TARGET = myTarget)
