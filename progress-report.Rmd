...
##17- Analysing the `GENDER` from `contract_ref` (Tony)
...
##18- Analysing the `VALUE_SEGMENT` from `contract_ref` (Tony)
...
##19- Analysing and extracting new features from `RATE_PLAN` (Tony)
...
```
We can see that there are some similarities within some of those different values that would reflect the same info/value. For example, *2013 Business El Asly 30* and *2014 Business El Asly 30* are two different `RATE_PLAN` values that mainly reflect the same rate plan but for different years. Hence, one of the things to be done, is to discard the year value from the `RATE_PAN`.