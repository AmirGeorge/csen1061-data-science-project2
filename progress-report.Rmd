...
```{r}
contractRefDf$RATE_PLAN[grepl(" [0-9]+(\\.[0-9]*)* GB", contractRefDf$RATE_PLAN)] %>% unique %>% length
```
Also the number of customers subscribed to those rate plans are also a small percentage (6.440326 %):
```{r}
contractRefDf$CONTRACT_KEY[grepl(" [0-9]+(\\.[0-9]*)* GB", contractRefDf$RATE_PLAN)] %>% length
contractRefDf$CONTRACT_KEY[grepl(" [0-9]+(\\.[0-9]*)* GB", contractRefDf$RATE_PLAN)] %>% length / contractRefDf %>% nrow * 100
```
...
##24- Adding the approaches of the 2 trials of extracting features from the `RATE_PLAN` in the `contractRefDf` (Tony)
So building on 23 and applying what was done in 18 by updating the `RATE_PLAN` as follows:
