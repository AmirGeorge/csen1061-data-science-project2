...
##1- Decision Tree classifier (Abanoub)
By trying on on the Rweka's J48 decision tree classifier on all features of the original train and test datasets, a kaggle score of `0.61` was obtained, and CV results as follows:
```{r cache=TRUE}
...
##2- Decision tree classifier with value segment and rate plan (Abanoub)
...
##7- Neural Network classifier (Abanoub)
...
evaluateModel(nnmodel)
rm(nnmodel)
```

##8- Random Forest classifier (Amir)
By trying on on the Random Forest (RF) classifier, a score of `0.60032` was obtained, which was less than that obtained from the Neural Network classifier, so this approach was neglected, and the NN classifier will be used from now on.
```{r cache=TRUE, eval=FALSE}
model <- RF(TARGET~., data=trainDf)
evaluateModel(model)
rm(model)
```
