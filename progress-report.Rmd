...
##TODO- Analysing and extracting new features from `RATE_PLAN` and adding it to the predictions - Trial 2 (Tony)
Let's try only excluding the year value from the `RATE_PLAN` and see whether the predicitons results would be postively/negatively affected.
```{r}
contractRefDf$RATE_PLAN <- gsub("[2][0][0-9][0-9] ", "", contractRefDf$RATE_PLAN)
contractRefDf$RATE_PLAN <- as.factor(contractRefDf$RATE_PLAN)
```
...
contractRefDf$RATE_PLAN %>% unique %>% length
```
Adding the `RATE_PLAN` to the predicitions, the score becomes ***Results***