...
It should be noted that the NN classifier which had the highest score on kaggle is the one with the highest precision in the 5-fold CV. Also, when studying the results of the SMO classifier, I was amused to find that the perfect recall score and the 0.79 precision are due to the fact that it recommended all target as `0`. It correctly classified all the 0-target observations as it classified all the observations as 0. Moreover, it scored a precision of 0.79 because this is the actual ratio of 0-target observations in the training set. From the misleading results of the SMO classifier, we deduce that the precision metric could be more valuable in our purpose, possibly due to the fact that most of the observations have a target of 0. However, some inspection of the classifier results in general are needed, as some high results due to overfitting can be misleading.
...
##-- Some insight into the data (Mimi)
```{r}
percentage_calculator <- function(target, threshold, colName){
    exp <- paste("(trainDf %>% filter(",colName,"< threshold & TARGET == target) %>% nrow() / trainDf %>% filter(",colName,"< threshold) %>% nrow()  ) * 100",sep='')
    eval(parse(text = exp))
}
range(trainDf$X210_USAGE)
```
 
The percentag of users who have X210_USAGE < 100MB to be target 0
```{r}
percentage_calculator(0,100,"X210_USAGE")
```
 
The percentag of users who have X210_USAGE < 500MB to be target 0
```{r}
percentage_calculator(0,500,"X210_USAGE")
```
 
The percentag of users who have X210_USAGE < 2000MB to be target 0
```{r}
percentage_calculator(0,2000,"X210_USAGE")
```
 
The number of users with target euqal to 1 become to overcome the users with target 0 when X210_USAGE execeds almost 4000 MB
```{r}
trainDf %>% filter(X210_USAGE > 4000 & TARGET == 1) %>% nrow
trainDf %>% filter(X210_USAGE > 4000 & TARGET == 0) %>% nrow
```

##-- Some insight into the data (Tony)
```{r}
trainDf <- trainDf %>% rowwise() %>% mutate(mean_usage_temp = mean(c(X206_USAGE,X207_USAGE,X208_USAGE,X209_USAGE,X210_USAGE)))
trainDf <- trainDf %>% rowwise() %>% mutate(mean_session_count_temp = mean(c(X206_SESSION_COUNT,X207_SESSION_COUNT,X208_SESSION_COUNT,X209_SESSION_COUNT,X210_SESSION_COUNT)))
trainDf <- trainDf %>% rowwise() %>% mutate(total_session_count_temp = X206_SESSION_COUNT+X207_SESSION_COUNT+X208_SESSION_COUNT+X209_SESSION_COUNT+X210_SESSION_COUNT)
```
The percentage of users whose mean data usage is < 100 MB to be target 0
```{r}
percentage_calculator(0,100,"mean_usage_temp")
```

The percentage of users whose mean data usage is < 500 MB to be target 0
```{r}
percentage_calculator(0,500,"mean_usage_temp")
```

The percentage of users whose mean data usage is < 1000 MB to be target 0
```{r}
percentage_calculator(0,1000,"mean_usage_temp")
```

The percentage of users whose mean session count is < 5 to be target 0
```{r}
percentage_calculator(0,5,"mean_session_count_temp")
```

The percentage of users whose mean session count is < 50 to be target 0
```{r}
percentage_calculator(0,50,"mean_session_count_temp")
```

The percentage of users whose mean session count is < 500 to be target 0
```{r}
percentage_calculator(0,500,"mean_session_count_temp")
```

The percentage of users whose total session count is < 50 to be target 0
```{r}
percentage_calculator(0,50,"total_session_count_temp")
```

The percentage of users whose total session count is < 500 to be target 0
```{r}
percentage_calculator(0,500,"total_session_count_temp")
```

The percentage of users whose total session count is < 1000 to be target 0
```{r}
percentage_calculator(0,1000,"total_session_count_temp")
```
...
labs(title = "The frequency of the Value Segments before cleaning/replacing the 'N/A's", x = "Value Segment", y = "Frequency")
```
...
```{r}
vs_frequencies <- contractRefDf %>% filter(RATE_PLAN == "Enterprise Shared Data Bundle", VALUE_SEGMENT != "N/A") %>% group_by(VALUE_SEGMENT) %>% summarise(VS_frequency = length(CONTRACT_KEY)) %>% arrange(desc(VS_frequency))
vs_frequencies
vs_frequencies %>% ggplot(aes(x = VALUE_SEGMENT, y = VS_frequency)) + geom_bar(stat='identity') +
...
```{r}
vs_frequencies <- contractRefDf %>% filter(RATE_PLAN == "2013 Business El Asly 30", VALUE_SEGMENT != "N/A") %>% group_by(VALUE_SEGMENT) %>% summarise(VS_frequency = length(CONTRACT_KEY)) %>% arrange(desc(VS_frequency))
vs_frequencies
vs_frequencies %>% ggplot(aes(x = VALUE_SEGMENT, y = VS_frequency)) + geom_bar(stat='identity') +
...
####Business Flex 35
```{r}
vs_frequencies <- contractRefDf %>% filter(RATE_PLAN == "Business Flex 35", VALUE_SEGMENT != "N/A") %>% group_by(VALUE_SEGMENT) %>% summarise(VS_frequency = length(CONTRACT_KEY)) %>% arrange(desc(VS_frequency))
vs_frequencies
vs_frequencies %>% ggplot(aes(x = VALUE_SEGMENT, y = VS_frequency)) + geom_bar(stat='identity') +
...
####2014 ElAsly Plus 35
```{r}
vs_frequencies <- contractRefDf %>% filter(RATE_PLAN == "2014 ElAsly Plus 35", VALUE_SEGMENT != "N/A") %>% group_by(VALUE_SEGMENT) %>% summarise(VS_frequency = length(CONTRACT_KEY)) %>% arrange(desc(VS_frequency))
vs_frequencies
vs_frequencies %>% ggplot(aes(x = VALUE_SEGMENT, y = VS_frequency)) + geom_bar(stat='identity') +
...
####Red 150
```{r}
vs_frequencies <- contractRefDf %>% filter(RATE_PLAN == "Red 150", VALUE_SEGMENT != "N/A") %>% group_by(VALUE_SEGMENT) %>% summarise(VS_frequency = length(CONTRACT_KEY)) %>% arrange(desc(VS_frequency))
vs_frequencies
vs_frequencies %>% ggplot(aes(x = VALUE_SEGMENT, y = VS_frequency)) + geom_bar(stat='identity') +
...
```{r}
vs_frequencies %>% ggplot(aes(x = VALUE_SEGMENT, y = VS_frequency)) + geom_bar(stat='identity') +
labs(title = "The frequency of the Value Segments after cleaning/replacing the 'N/A's", x = "Value Segment", y = "Frequency")
```
...
Let's try to extract the monetary value from the `RATE_PLAN` and then include it in the predictions, in a new column/attribute `RATE_PLAN_VALUE`.
...
```
Now, any numerical value existing in the `RATE_PLAN` is the monetary value we want.
...
contractRefDfTemp$RATE_PLAN <- as.factor(contractRefDfTemp$RATE_PLAN)
```
So adding both the updated `RATE_PLAN` and the `RATE_PLAN_VALUE` (calculated in 33) together to the predictions, we obtain a minimum score of `0.50000`.
...
Some of the `RATE_PLAN_VALUE` values have a combination of plans. Hence, at this stage  for such examples, the `RATE_PLAN_VALUE` might have two monetary values; such as '30 150'. So let's extract the numerical values out and get their mean. The single values will remain as they are and the non numerical values (the rate plans that didn't contain any monetary value such as `Enterprise Shared Data Bundle`) will have their `RATE_PLAN_VALUE` as NAs.
```{r warning=FALSE}
...
labs(title = "Most frequent rate plan (monetary) values", x = "Rate Plan Value", y = "Average Data Usage")
...
labs(title = "Average Data Usage for all users for each rate plan (monetary) value", x = "Rate Plan Value", y = "Total Average Data Usage")
```

Let's have a more generalized feature for the average data usage to help us spot the relation between the `RATE_PLAN_VALUE` and the amount of data usage. So let's round the calculated `mean_usage` to the nearest 100 MB:
```{r}
...
As we can see, the dominant rate plan (monetary) values for the corresponding (< 50 MB [0] avg. usage) are the `30` and `35` ones. Both are quite close values. Hence, it's showing a relation between the `RATE_PLAN_VALUE` and the corresponding average data usage.
...
As we can see, the dominant rate plan (monetary) values for the corresponding (apprx. 200 MB avg. usage) is `35`. Hence, it's showing a relation between the `RATE_PLAN_VALUE` and the corresponding average data usage.
...
As we can see, the dominant rate plan (monetary) values for the corresponding (apprx. 100 MB avg. usage) is `35`. Hence, it's showing a relation between the `RATE_PLAN_VALUE` and the corresponding average data usage.
...
##-- MLP with different model parameters (Mimi & Amir)
The MLP that scored the highest score so far `0.70333` (in step 26) had a learning rate of `0.3`. When building the model with a lesser learning rate of `0.2`, a decreased score of `0.70168` was obtained. The reason for this could be that the lesser learning rate caused the network to reach a local minimum when training, hence the neural network was not trained in the optimal way.
...
To recap our previous work, we have added many features that have increased our kaggle score, and temporarily added some features that unfortunately did not and ultimately removed them. In some of the intermediate steps, we have relied on the kaggle scoring to determine the usefulness of features, as performing cross validation in every minor step was time-consuming and unpractical. Working throughout a long period of time permiited us to make use of a large number of submissions, so CV was done in some of the steps only.
Using the chosen final features, I have run a one time 5-fold cross validation on my local machine, using these features and 4 diferrent classifiers: 
...
Same as when using the initial features, the SMO classifier achieves the highest F1 score due to its almost perfect recall score. Our experience with CV on the initial features indicates that this is probably due to overfitting This is confirmed the fact that the `SMO` classifier scored the least scores on kaggle among the rest of the classifiers.
Also, we note that that our chosen MLP classifier scored only the second best precision after the J48 classifier. But, when trying a submission using our final features, a significantly decreased score of `0.59285 ` was obtained, which is evidence that the higher CV kaggle scores is probably due to overfitting as well.
Generally, these discrepancies could be attributed to using one-time 5-fold CV only (due to computational time constraints). As a results, we made use of evaluation results from Microsoft Azure server as well for better insights.

#-- CrossValidation between different models on Azure server (Mimi)
At different steps, we carried on cross validation using our chosen features on some models on Microsoft Azure. It was unpractical to fully rely on local CV at each minor modification due to the exceedingly long computational times. Below is a demonstration of Azure CV results using 4 sets of features as described:

## Cross validation for the first train columns:
This is using all the original features from the original train dataset and before integrating with roaming and daily data).
...
## Cross validation for the model after extracing the roaming features
As illustrated in script & step `13`.
...
Cross validation for the model after extracting the daily features.: (Script & Step 25)
...
## Cross validation for the model after adding weekday and weekend features
As illustrated in step and script `26`.
...
Note that the Neural Network has the most accuracy. However we need to check the precision to get a better indicator of the better classifier.
...
```
We can see that the precision of Neural Networks increases as we add our new features and adding them achieves the best increase in precision among the other classifiers.
...
## Notes on model parameter selection in Azure

For Random forest classifiers we set the the number of tree in the range of (64-128) according to this [article](https://www.researchgate.net/publication/230766603_How_Many_Trees_in_a_Random_Forest) . The goal from this was to balance between the performance of results and the computational time in training.

In general, when we used Azure classsifers We used range parameters, to give the classifier a better chance at calculating the optimal parameters.


## Evaluation by splitting data on Azure server (Mimi)
In addition to the CV we did on Azure server, we did another evaluation on Azure of the neural network model by splitting the train data into stratified samples where the data to be trained is 70% of the whole data.
```{r}
evalModel <- read.csv('CrossValidationsDataFramesAzure/evaluateModel.csv')
evalModel %>% kable
```

# Conclusion (Amir)
Throughout the whole course of our work, we have utilized different features. We had an initial thought that the usage only would be of importance. However, the session count data proved useful as well and factoring it caused better predictions.
Although the roaming data was covering only a small percentage of the users in both datasets, our predictions were improved by adding features extracted from it. We believe that the relatively small availability of roaming data is attributed to the fact that using roaming mobile data is indeed relatively rare compared to local data.
Last but not least, features extracted from the daily aggregate data of the last month slightly improved our predictions as well. It was impractical to add features representing the usage and session count for each of the 30 days. So, instead, we added features representing each of the seven days of the week. Last but not least, adding features representing usage and session count for the weekends and weekdays slightly improved our results. This points to a relation between user behaviour and the day being a weekday or weekend.
